...
class PDF_v3 extends Controller
{
    ...
    public function __construct(OrderRepository $OrderRepository)
    {
        ...
        $this->ewoTools = new ewoToolsController();
    }
    ...
    public function createPDF($source,$version,$orderId,$params='')
    {
        ...
        try {
            ...
            if (empty($orderId) || empty($workKind)) {
                ...
                $ms0301ByAssignSheet = $this->OrderRepository->getWorksheetList($whereAry);
                if(!isset($ms0301ByAssignSheet[0]))
                    throw new Exception('查無工單資訊','0513');
                ...
                $brokerKind = data_get($ms0301ByAssignSheet,'BrokerKind');
                $workKind = data_get($ms0301ByAssignSheet,'WorkKind');

                // pdf 版本、條款版本
                list($pdf_v,$pdfTerms) = $this->ewoTools->getPDFParams(['bookDate'=>$bookDate,'workKind'=>$workKind]);

                // 檢查 遠傳 & bookdate >= 2023-03-25； 網址跳轉 v3fet
                if($brokerKind == '789 遠傳' && substr($bookDate,0,10) >= '2023-03-25') {
                    $pdf_v = config('order.PDF_CODE_FET_V');
                    $url = "/api/createpdf/web/$pdf_v/NULL-$companyNo-$workSheet?cmd=Y";
                    header("Location: $url");
                    exit('createPdf(code:109)');
                }
            ...
            if($chkCmd == 'Y') {
                ...
                $chgCompanyo = $brokerKind == '789 遠傳'? 'fet' : $companyNo;
    ...
    //            $data['RetrieveListDevJson'] = json_encode($data['RetrieveListDev']);
            ...
            if(TRUE)
            {
                ...
                if (in_array($workKind,array('1 裝機','6 移機','A 加裝','C 換機')) && strlen($serviceName2) > 0) {
                    ...
                    // [I]，速率 月/金 違約金 期限
                    $dataList = data_get($order_info,'dataList');
                    $backUpPDF = data_get(json_decode($dataList,true),'backUpPDF');
                    if(empty($backUpPDF)) {
                        ...
                        $objRet = $objTools->getProgramInfo($objAry);
                    } else {
                        $objRet = array(
                            'BillItem' => data_get($backUpPDF,'BillItem'),
                            'Aveamt' => data_get($backUpPDF,'Aveamt'),
                            'PackDuration' => data_get($backUpPDF,'PackDuration'),
                            'PenalAmt01' => data_get($backUpPDF,'PenalAmt01'),
                            'BookDate' => data_get($backUpPDF,'BookDate'),
                        );
                    }

                    $billItem01 = data_get($objRet,'BillItem');
                    $aveamt = data_get($objRet,'Aveamt');
                    $packDuration = data_get($objRet,'PackDuration');
                    $penalAmt01 = data_get($objRet,'PenalAmt01');
                    $objBookDate = data_get($objRet,'BookDate');
                    $emptyListPriceORListPrd = data_get($objRet,'emptyListPriceORListPrd');
                    $data['emptyListPriceORListPrd'] = $emptyListPriceORListPrd;
                    ...
                    data_set($order_info,'BookDate',$objBookDate);
...
//            }
            ...
            return view('pdf.table_v3',compact('data'));
...
//        $ms0301List = $this->OrderRepository->getOrderCharge($chargeData);
        ...
        foreach($ms0301List as $k => $t) {
            ...
            if(!in_array($vChargeKind,['12','40','50','57','4A'])) continue;
